...
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  if (
    ...
    currentMessageContent.includes(
      "Rejoins Archie dans son enquête en cliquant sur l'emoji 🔍 en dessous."
    )
  ) {
    user.send(
      `Bienvenu dans mon agence ${user.username} !\nTa mission actuelle est: **${memberCurrentQuest.name}**`
    );
  }
...
client.on("messageCreate", async (message) => {
  ...
  if (!message.guild) {
    ...
    const currentScenarioMessage = memberCurrentQuest.scenario.find(
      (message) => message.expectedResponses
    );
    ...
    if (messageIsCorrect(currentScenarioMessage, message)) {
      ...
      await sendWrongResponseMessage(
        member,
  ...
  if (message.guild) {
    if (
      message.content.includes("Archie") ||
      message.content.includes("archie") ||
      message.content.includes("archi") ||
      message.content.includes("Leguillon") ||
      message.content.includes("leguillon")
    ) {
